...
function getFilterMenu(event,colonna,icon)
{
    ...
    checkboxButton.setAttribute("class","filter-menu-item-report-ordini-cliente");
    checkboxButton.setAttribute("onclick","selectAllFilterArrayItems('"+colonna+"',this)");

    var i=document.createElement("i");
    i.setAttribute("class","fas fa-check-square filter-menu-item-report-ordini-cliente");
    
    checkboxButton.appendChild(i);

    var span=document.createElement("span");
    span.setAttribute("class","filter-menu-item-report-ordini-cliente");
    span.innerHTML="Tutti";
    checkboxButton.appendChild(span);

    filterInnerContainer.appendChild(checkboxButton);

    filterArray=filterArrays[colonna];
    filterArray.forEach(function(item)
    {
        var checkboxButton=document.createElement("button");
        checkboxButton.setAttribute("class","filter-menu-item-report-ordini-cliente filterMenuItemCheckboxButton"+colonna);
        checkboxButton.setAttribute("item",item);
        ...
        ordini.forEach(function(ordine)
        {
            //console.log(ordine[colonna]+" - "+item)
            if(ordine[colonna]==item)
                checked=true;
        });
        if(checked)
        {
            checkboxButton.setAttribute("onclick","removeFilterArrayItem('"+colonna+"','"+item+"',this)");
            i.setAttribute("class","fas fa-check-square filter-menu-item-report-ordini-cliente");
        }
        else
        {
            checkboxButton.setAttribute("onclick","addFilterArrayItem('"+colonna+"','"+item+"',this)");
            i.setAttribute("class","fal fa-square filter-menu-item-report-ordini-cliente");
        }
        checkboxButton.appendChild(i);
    ...
    filterConfirmButton.setAttribute("onclick","applyFilter()");
    var span=document.createElement("span");
    span.setAttribute("class","filter-menu-item-report-ordini-cliente");
    span.innerHTML="Conferma";
    filterConfirmButton.appendChild(span);
    var i=document.createElement("i");
    i.setAttribute("class","filter-menu-item-report-ordini-cliente");
...
function selectAllFilterArrayItems(colonna,icon)
{
    /*var all=document.getElementsByClassName("filterMenuItemCheckboxButton"+colonna);
    for (let index = 0; index < all.length; index++) {
        const element = all[index];
        addFilterArrayItem(colonna,element.getAttribute("item"),element);
    }*/
}
function removeFilterArrayItem(colonna,item,button)
{
    var icon=button.getElementsByTagName("i")[0];
    icon.className="fal fa-square filter-menu-item-report-ordini-cliente";
    console.log(ordini.length);
    var tempOrdini=ordini;
    ordini=[];
    for (let index = 0; index < tempOrdini.length; index++)
    {
        const ordine = tempOrdini[index];
        if(ordine[colonna]!==item)
        {
            ordini.push(ordine);
        }
    }
    button.setAttribute("onclick","addFilterArrayItem('"+colonna+"','"+item+"',this)");
    console.log(ordini.length);
}
function addFilterArrayItem(colonna,item,button)
{
    var icon=button.getElementsByTagName("i")[0];
    icon.className="fas fa-check-square filter-menu-item-report-ordini-cliente";
    console.log(ordini.length);
    var tempOrdini=ordiniOriginal;
    console.log(tempOrdini)
    //ordini=[];
    for (let index = 0; index < tempOrdini.length; index++)
    {
        const ordine = tempOrdini[index];
    ...
    button.setAttribute("onclick","removeFilterArrayItem('"+colonna+"','"+item+"',this)");
    console.log(ordini.length);
}
