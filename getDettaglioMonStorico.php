<?php
	...
	{
		$query2="SELECT * FROM st_dettaglioMon WHERE settimana='$week' AND stazione='$stazione' ORDER BY docnum";	
	...
	{
		$query2="SELECT [docnum],[stazione],ISNULL([totale_pezzi],0) AS totale_pezzi,ISNULL([basi_portalavabo],0) AS basi_portalavabo,ISNULL([colonne],0) AS colonne,ISNULL([pensili],0) AS pensili,ISNULL([basi_accostabili],0) AS basi_accostabili,ISNULL([Altro],0) AS Altro,[settimana] FROM st_dettaglioMon_2 WHERE settimana='$week' AND stazione='$stazione' ORDER BY docnum";	
		$result2=sqlsrv_query($conn,$query2);
		if($result2==FALSE)
		{
			echo "<br><br>Errore esecuzione query<br>Query: ".$query2."<br>Errore: ";
