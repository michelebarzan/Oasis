<?php
	...
	if(set_time_limit(120))
	{
		...
		echo "<div id='containerResiduoGestioneSettimane' class='colonnaContainerGestioneSettimane'>";
			...
			echo "<div id='containerResiduoVER' class='containerResiduo'>";
				echo "<div id='residuoVER' class='ordiniVER'>";
	...
	function getOrdiniResiduo($conn,$elem,$stazione,$stazioneVisualizzata)
	{
		...
		$query2="SELECT TOP (100) PERCENT dbo.$stazioneVisualizzata.docnum, dbo.$stazioneVisualizzata.stazione, ISNULL(dbo.$stazioneVisualizzata.totale_pezzi, 0)  AS totale_pezzi, ISNULL(dbo.$stazioneVisualizzata.basi_portalavabo, 0) AS basi_portalavabo, ISNULL(dbo.$stazioneVisualizzata.colonne, 0) AS colonne,  ISNULL(dbo.$stazioneVisualizzata.pensili, 0) AS pensili, ISNULL(dbo.$stazioneVisualizzata.basi_accostabili, 0) AS basi_accostabili,  ISNULL(dbo.$stazioneVisualizzata.Altro, 0) AS Altro, ISNULL(dbo.$stazioneVisualizzata.mq, 0) AS mq, dbo.$stazioneVisualizzata.settimana,  dbo.$stazioneVisualizzata.dataConsegna, dbo.colori_collezioni.colore, dbo.$stazioneVisualizzata.collezione FROM            dbo.colori_collezioni RIGHT OUTER JOIN dbo.$stazioneVisualizzata ON dbo.colori_collezioni.collezione = dbo.$stazioneVisualizzata.collezione WHERE        (dbo.$stazioneVisualizzata.settimana < '$week') AND (dbo.$stazioneVisualizzata.stazione = '$stazione') ORDER BY dbo.$stazioneVisualizzata.docnum";	
		$result2=sqlsrv_query($conn,$query2);
		if($result2==FALSE)
		{
			echo "<br><br>Errore esecuzione query<br>Query: ".$query2."<br>Errore: ";
		...
		else
		{
			...
			if ($rows === true)
			{
				...
				while($row2=sqlsrv_fetch_array($result2))
				{
					...
					$week='Residuo';
					echo '<div id="'.$i.'elementOrdineResiduo" class="elementOrdine" draggable="true" ondragend="fixContainerStyle()" ondragstart="drag(event)" ondrop="blockDrop(event)" onclick="'. 'apriPopupDettaglioOrdine(this.id,'. htmlspecialchars(json_encode($week)) . ')" style="background:#'.$row2["colore"].'">';
	...
	function getOrdiniWeek($conn,$elem,$stazione,$stazioneVisualizzata)
	{
		$elem2=explode("_",$elem);
		$week=$elem2[0].$elem2[1];
		$query2="SELECT TOP (100) PERCENT dbo.$stazioneVisualizzata.docnum, dbo.$stazioneVisualizzata.stazione, ISNULL(dbo.$stazioneVisualizzata.totale_pezzi, 0)  AS totale_pezzi, ISNULL(dbo.$stazioneVisualizzata.basi_portalavabo, 0) AS basi_portalavabo, ISNULL(dbo.$stazioneVisualizzata.colonne, 0) AS colonne, ISNULL(dbo.$stazioneVisualizzata.pensili, 0) AS pensili, ISNULL(dbo.$stazioneVisualizzata.basi_accostabili, 0) AS basi_accostabili,  ISNULL(dbo.$stazioneVisualizzata.Altro, 0) AS Altro, ISNULL(dbo.$stazioneVisualizzata.mq, 0) AS mq, dbo.$stazioneVisualizzata.settimana,  dbo.$stazioneVisualizzata.dataConsegna, dbo.colori_collezioni.colore, dbo.$stazioneVisualizzata.collezione FROM            dbo.colori_collezioni RIGHT OUTER JOIN dbo.$stazioneVisualizzata ON dbo.colori_collezioni.collezione = dbo.$stazioneVisualizzata.collezione WHERE (dbo.$stazioneVisualizzata.settimana = '$week') AND (dbo.$stazioneVisualizzata.stazione = '$stazione')  ORDER BY dbo.$stazioneVisualizzata.posizione";	
		//echo $query2;
		$result2=sqlsrv_query($conn,$query2);
		if($result2==FALSE)
		{
