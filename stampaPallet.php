			...
			<script>
				...
				function stampa() 
				{
					...
					if (myElem == true) 
						...
						window.location = 'anteprimaDiStampa.php';
				}
	...
	<body>
		...
		<div id="container">
			...
			<div id="content">
				...
				<form name="stampaPallet" id="stampaPallet" class="stampaPallet" action="stampaPallet.php" method="POST">
					<span style='font-family:arial;font-size:100%;color:gray;font-weight:bold;'>Elenco pallet&nbsp</span>
					<?php
					echo '<select name="N_Pick" id="selectN_Pick" onChange="document.stampaPallet.submit();" style="padding: 0.5% ;display: inline-block;border: 1px solid #CFD1DC;border-radius: 5px;box-sizing: border-box; color:#66B2FF;font-family:arial;font-size:90%;font-weight:bold;"/>';
						
					if($N_Pick==NULL)
						echo '<option value="" disabled selected>Scegli N_Pick</option>';
					else
					{
						$queryDescr="SELECT DISTINCT T_Picking_01.DescrPick FROM T_Picking_01 WHERE N_Pick=$N_Pick";
						$resultDescr=sqlsrv_query($conn,$queryDescr);
						if($resultDescr==FALSE)
						{
							echo "<br><br>Errore esecuzione query<br>Query: ".$queryDescr."<br>Errore: ";
							die(print_r(sqlsrv_errors(),TRUE));
						}
						else
						{
							while($rowDescr=sqlsrv_fetch_array($resultDescr))
							{
								$Descr2=$rowDescr['DescrPick'];
							}
						}
						echo "<option value=$N_Pick>$N_Pick | $Descr2 | Stato: ".stato($conn,$N_Pick).",&nbsp".controllaChecked($conn,$N_Pick)."</option>";
					}
					if($N_Pick==NULL)
						$query2="SELECT DISTINCT top(500) T_Picking_01.N_Pick, T_Picking_01.DataPick,T_Picking_01.DescrPick FROM T_Picking_01 ORDER BY T_Picking_01.N_Pick DESC";
					else
						$query2="SELECT DISTINCT top(500) T_Picking_01.N_Pick, T_Picking_01.DataPick,T_Picking_01.DescrPick FROM T_Picking_01 WHERE T_Picking_01.N_Pick<>$N_Pick ORDER BY T_Picking_01.N_Pick DESC";	
					...
					else
					{
						...
						{
							$N_PickS=$row2['N_Pick'];
							$DescrPick=$row2['DescrPick'];
							
							echo "<option value=$N_PickS>".$N_PickS." | ".$DescrPick." | Stato: ".stato($conn,$N_PickS).",&nbsp".controllaChecked($conn,$N_PickS)."</option>";
						}
					}
					echo "</select>";
				...
				<span id="stampaPalletB" class="stampaPalletB" ><input type="submit" onClick="stampa()" value="Impagina stampa"></span>
				<br><br>
					...
					if($N_Pick!=NULL)
					{
						...
						else
						{
							...
							if ($rows === TRUE)
							{
								...
								creaEriempiTabella($conn,$N_Pick,$Descr2);
							}
							else
...
function riempiTabella($conn,$N_Pick,$Descr2)
{
